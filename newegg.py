...
def extract_price(data):
    res = sorted(float(x.replace(',', ''))
                 for x in re.findall('\$([,0-9.]+)', data))
    if not res:
        raise ValueError('no price in %r' % (data,))
...
def get(N, *PropertyCodeValues):
    ...
    while True:
        ...
        for item in dom.findAll('div', {'class': 'itemCell'}):
            ...
            title += " " + \
                extract_text(item.find('ul', {'class': 'itemFeatures'}))
            ...
            try:
                out = re.compile(
                    "[^0-9A-Za-z.]([0-9.]+)( )?([MGT])B").findall(title.upper())[0]
            except IndexError:
            ...
            try:
                ...
                try:
                    price = float(
                        extract_text(item.find('li', {'class': 'price-current'})).split('$')[1].replace(',', ''))
                except:
                    traceback.print_exc()
                    try:
                        price = float(
                            extract_text(item.find('li', {'class': 'priceBefore'})).split('$')[1].replace(',', ''))
                    except:
                        traceback.print_exc()
                        # print "invalid price",
                        # repr(extract_text(item.find('li', {'class':
                        # 'priceFinal'}))), repr(extract_text(item.find('li',
                        # {'class': 'priceList'})))
                        continue
